...
const HealthForm = ({ empNo }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const fitnessRes = await axios.get("http://localhost:3000/master/fitness");
  ...
  return (
    <div className="space-y-6">
      <input type="hidden" {...register("health.emp_no")} value={empNo} /> {/* Auto-filled */}
      ...
      <div className="p-4 bg-gray-50 rounded">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              {...register("health.blood_group", { required: "Blood Group is required" })}
              className={`w-full p-2 border rounded ${
                errors.health?.blood_group ? "border-red-500" : "border-gray-300"
              }`}
            />
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium mb-1">Fitness Category</label>
            <select
              {...register("health.fitness_id", { required: "Fitness is required" })}
              className={`w-full p-2 border rounded ${
      ...
      <div className="p-4 bg-blue-50 rounded">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input
              type="date"
              {...register("medical_history.check_date", { required: "Check Date is required" })}
              className={`w-full p-2 border rounded ${
                errors.medical_history?.check_date ? "border-red-500" : "border-gray-300"
              }`}
            />
          ...
          <div>
            ...
            <input
              type="date"
              {...register("medical_history.expire_date", { required: "Expire Date is required" })}
              className={`w-full p-2 border rounded ${
                errors.medical_history?.expire_date ? "border-red-500" : "border-gray-300"
              }`}
            />
          ...
          <div>
            ...
            <input
              ...
              className={`w-full p-2 border rounded ${
                errors.medical_history?.status ? "border-red-500" : "border-gray-300"
              }`}
              maxLength={10}
            />
